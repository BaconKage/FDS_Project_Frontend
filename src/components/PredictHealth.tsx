...
export default function PredictHealth() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (typeof data.bmi === "number") setBmi(data.bmi);

      // ✅ save for Dashboard (/compare)
      localStorage.setItem("lastSubmission", JSON.stringify(payload));
      // console.log("Saved lastSubmission:", JSON.parse(localStorage.getItem("lastSubmission") || "{}"));
    } catch (err: any) {
      setErrorText(String(err?.message || err) || "Error connecting to backend");
      setPrediction(null);
      setBmi(null);
    } finally {
      setIsLoading(false);
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        ...
        <p className="text-sm text-gray-500">
          Backend: <code className="px-1.5 py-0.5 bg-gray-100 rounded">{API}</code>
        </p>
      </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              {bmi !== null && (
                <div className="mb-3 text-sm text-gray-700">
                  <span className="font-medium">BMI:</span> {bmi.toFixed(2)}
                </div>
              )}
              {prediction && (
